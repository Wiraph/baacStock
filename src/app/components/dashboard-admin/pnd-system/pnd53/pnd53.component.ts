...
export class Pnd53Component implements OnInit {
  ...
  constructor(private http: HttpClient) {
    console.log('PND53 Component constructor called');
  }

  ngOnInit(): void {
    console.log('PND53 ngOnInit called');
    this.loadPNDData();
  }

  /**
   * โหลดข้อมูล PND53 จาก API
   */
  loadPNDData(): void {
    // TODO: เรียก API จริง
    // this.isLoading = true;
    // this.http.get(`${environment.apiUrl}/pnd/pnd53/list`).subscribe({
    //   next: (response: any) => {
    //     this.pndData = this.mapApiResponse(response);
    //     this.isLoading = false;
    //   },
    //   error: (error) => {
    //     console.error('Error loading PND53 data:', error);
    //     this.isLoading = false;
    //   }
    // });
    ...
    this.isLoading = false;
    console.log('PND53 Data loaded:', this.pndData);
  }

  /**
   * สร้างไฟล์ PND53 สำหรับเดือนที่เลือก
   */
  ...
  private getMockData(): PNDData[] {
    ...
    // ข้อมูล Mock ตามรูปแบบจริง (เหมือน PND2)
    const mockPeriods = [
  ...
  private generateMockFiles(yearMonth: string): PNDFile[] {
    ...
    const baseFileName = `PND53_0994000164912_000000_${formattedDate}`;
    
    return [
      {
        name: `${baseFileName}_EFL.txt`,
        type: 'EFL',
        path: `pnd53/${baseFileName}_EFL.txt`,
        available: true,
        size: '12 KB'
      },
      {
        ...
        type: 'SWC',
        path: `pnd53/${baseFileName}_SWC.txt`,
        available: true,
        size: '14 KB'
      },
      {
        ...
        type: 'XLS',
        path: `pnd53/${baseFileName}_XLS.xlsx`,
        available: true,
        size: '22 KB'
      }
    ];
